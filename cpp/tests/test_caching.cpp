...
TEST_CASE("caching: cache hit returns same record on repeated get", "[cache][caching]") {
    auto at = cached();
    auto created = at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "Hit")});
    const auto id = *created.id;
    try {
        auto first = at.primary().get(id);
        auto second = at.primary().get(id);
...
TEST_CASE("caching: cache is disabled when cache seconds is zero", "[cache][caching]") {
    auto at = uncached();
    REQUIRE(at.client()->cache().size() == 0);
    auto created = at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "Off")});
    const auto id = *created.id;
    try {
        at.primary().get(id);
...
TEST_CASE("caching: create invalidates cached reads for the table", "[cache][caching]") {
    ...
    auto a = at.primary().create(PrimaryModel{.primary_key = key});
    const auto id_a = *a.id;
    std::string id_b;
    try {
        at.primary().get(id_a); // populate cache
        REQUIRE(at.client()->cache().size() > 0);

        auto b = at.primary().create(PrimaryModel{.primary_key = key + " B"});
        id_b = *b.id;
...
TEST_CASE("caching: update invalidates cached reads for the table", "[cache][caching]") {
    auto at = cached();
    auto created =
        at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "MutateUpdate")});
    const auto id = *created.id;
    try {
        at.primary().get(id);
        REQUIRE(at.client()->cache().size() > 0);
...
TEST_CASE("caching: delete invalidates cached reads for the table", "[cache][caching]") {
    auto at = cached();
    auto created =
        at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "MutateDelete")});
    const auto id = *created.id;
    bool deleted = false;
    try {
        at.primary().get(id);
...
TEST_CASE("caching: invalidate cache drops this table's cache", "[cache][caching]") {
    auto at = cached();
    auto created =
        at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "ManualTable")});
    const auto id = *created.id;
    try {
        at.primary().get(id);
        REQUIRE(at.client()->cache().size() > 0);
...
TEST_CASE("caching: invalidate all caches drops every table's cache", "[cache][caching]") {
    auto at = cached();
    auto created =
        at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "ManualAll")});
    const auto id = *created.id;
    try {
        at.primary().get(id);
        at.secondary().get_all(AirtableQuery{.max_records = 1});
...
TEST_CASE("caching: different query params produce different cache keys", "[cache][caching]") {
    auto at = cached();
    auto created = at.primary().create(PrimaryModel{.primary_key = primary_key("Cache", "Keys")});
    const auto id = *created.id;
    try {
        at.primary().get_all(AirtableQuery{.max_records = 1});
        at.primary().get_all(AirtableQuery{.max_records = 2});
