...
void try_remove(Airtable& airtable, const std::string& record_id) {
    try {
        airtable.primary().dict().delete_one(record_id);
...
TEST_CASE("struct crud: primary-key-only round trip", "[crud][struct-crud]") {
    ...
    const auto created = airtable.primary().dict().create_one(fields);
    ...
    try {
        ...
        const auto fetched = airtable.primary().dict().get_one(record_id);
        ...
        const auto updated = airtable.primary().dict().update_one(record_id, update);
        REQUIRE(updated.fields.get_string(PrimaryFields::kPrimaryKeyId) == updated_pk);

        airtable.primary().dict().delete_one(record_id);
        REQUIRE_THROWS_AS(airtable.primary().dict().get_one(record_id), AirtableException);
    } catch (...) {
...
TEST_CASE("struct crud: all simple properties round trip", "[crud][struct-crud]") {
    ...
    const auto created = airtable.primary().dict().create_one(fields);
    try {
        const auto fetched = airtable.primary().dict().get_one(created.id);
