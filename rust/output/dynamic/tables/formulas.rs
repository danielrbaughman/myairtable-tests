...
use crate::models::{CreateFormulas, Formulas};
use crate::pagination::PaginatedResponse;
use crate::types::{Record, RecordId};

/// Table wrapper for `Formulas`
pub struct FormulasTable<'a> {
    client: &'a AirtableClient,
}

impl<'a> FormulasTable<'a> {
    /// Airtable table ID.
    pub const TABLE_ID: &'static str = "tblnuYBsMdXNDsuRc";
    /// Airtable table name.
    pub const TABLE_NAME: &'static str = "Formulas";
    ...
    pub async fn list(
        &self,
        offset: Option<&str>,
    ) -> Result<PaginatedResponse<Formulas>, AirtableError> {
        self.client.list_records(Self::TABLE_ID, offset).await
    }

    /// Get a single record by ID.
    pub async fn get(&self, record_id: &RecordId) -> Result<Record<Formulas>, AirtableError> {
        self.client.get_record(Self::TABLE_ID, record_id).await
    }

    /// Create a new record.
    pub async fn create(&self, fields: &CreateFormulas) -> Result<Record<Formulas>, AirtableError> {
    ...
    pub async fn create_many(
        &self,
        records: &[CreateFormulas],
    ) -> Result<Vec<Record<Formulas>>, AirtableError> {
    ...
    pub async fn update(
        &self,
        record_id: &RecordId,
        fields: &CreateFormulas,
    ) -> Result<Record<Formulas>, AirtableError> {
        self.client
    ...
    pub async fn update_many(
        &self,
        records: &[(&RecordId, &CreateFormulas)],
    ) -> Result<Vec<Record<Formulas>>, AirtableError> {
