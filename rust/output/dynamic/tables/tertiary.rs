...
use crate::models::{CreateTertiary, Tertiary};
use crate::pagination::PaginatedResponse;
use crate::types::{Record, RecordId};

/// Table wrapper for `Tertiary`
pub struct TertiaryTable<'a> {
    client: &'a AirtableClient,
}

impl<'a> TertiaryTable<'a> {
    /// Airtable table ID.
    pub const TABLE_ID: &'static str = "tblLFoLxEdWlxjmLP";
    /// Airtable table name.
    pub const TABLE_NAME: &'static str = "Tertiary";
    ...
    pub async fn list(
        &self,
        offset: Option<&str>,
    ) -> Result<PaginatedResponse<Tertiary>, AirtableError> {
        self.client.list_records(Self::TABLE_ID, offset).await
    }

    /// Get a single record by ID.
    pub async fn get(&self, record_id: &RecordId) -> Result<Record<Tertiary>, AirtableError> {
        self.client.get_record(Self::TABLE_ID, record_id).await
    }

    /// Create a new record.
    pub async fn create(&self, fields: &CreateTertiary) -> Result<Record<Tertiary>, AirtableError> {
    ...
    pub async fn create_many(
        &self,
        records: &[CreateTertiary],
    ) -> Result<Vec<Record<Tertiary>>, AirtableError> {
    ...
    pub async fn update(
        &self,
        record_id: &RecordId,
        fields: &CreateTertiary,
    ) -> Result<Record<Tertiary>, AirtableError> {
        self.client
    ...
    pub async fn update_many(
        &self,
        records: &[(&RecordId, &CreateTertiary)],
    ) -> Result<Vec<Record<Tertiary>>, AirtableError> {
