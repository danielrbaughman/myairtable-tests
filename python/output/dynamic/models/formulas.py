...
class FormulasModel(Model):
    ...
    def date_formula(self, recalculate: bool = False) -> str | None:
        if recalculate:
            self._fields["fldY7kjaklLeoSgGd"] = ((((((((((((((((((((((((((((((((((((((((((((("YEAR: " + F.S(F.D(self.first_date).year)) + ", MONTH: ") + F.S(F.D(self.first_date).month)) + ", DAY: ") + F.S(F.D(self.first_date).day)) + ", HOUR: ") + F.S(F.D(self.second_date).hour)) + ", MINUTE: ") + F.S(F.D(self.second_date).minute)) + ", SECOND: ") + F.S(F.D(self.second_date).second)) + ", TODAY: ") + F.S(F.DATETIME_FORMAT(F.TODAY(), 'YYYY-MM-DD'))) + ", WORKDAY+5: ") + F.S(F.DATETIME_FORMAT(F.WORKDAY(self.first_date, 5), 'YYYY-MM-DD'))) + ", WORKDAY_DIFF: ") + F.S(F.WORKDAY_DIFF(self.first_date, self.second_date))) + ", PARSE: ") + F.S(F.DATETIME_FORMAT(F.DATETIME_PARSE(F.DATESTR(self.first_date)), 'YYYY-MM-DD'))) + ", FORMAT: ") + F.S(F.DATETIME_FORMAT(self.second_date, 'YYYY-MM-DD HH:mm'))) + ", LOCALE: ") + F.S(F.DATETIME_FORMAT(F.SET_LOCALE(self.second_date, 'en'), 'YYYY-MM-DD'))) + ", TIMEZONE: ") + F.S(F.DATETIME_FORMAT(F.SET_TIMEZONE(self.second_date, 'America/New_York'), 'YYYY-MM-DD HH:mm'))) + ", DATESTR: ") + F.S(F.DATESTR(self.second_date))) + ", TIMESTR: ") + F.S(F.TIMESTR(self.second_date))) + ", TONOW: ") + F.S(F.TONOW(self.first_date))) + ", FROMNOW: ") + F.S(F.FROMNOW(self.second_date))) + ", DATEADD+2d: ") + F.S(F.DATETIME_FORMAT(F.DATEADD(self.first_date, 2, 'days'), 'YYYY-MM-DD'))) + ", WEEKNUM: ") + F.S(F.WEEKNUM(self.first_date, 'Monday'))) + ", DIFF days: ") + F.S(F.DATETIME_DIFF(self.first_date, self.second_date, 'days'))) + ", IS_BEFORE: ") + F.S(('Yes' if F.IS_BEFORE(self.first_date, self.second_date) else 'No'))) + ", IS_SAME day: ") + F.S(('Yes' if F.IS_SAME(self.first_date, self.second_date, 'day') else 'No'))) + ", IS_AFTER: ") + F.S(('Yes' if F.IS_AFTER(self.third_date, self.second_date) else 'No')))
    ...
    def text_formula(self, recalculate: bool = False) -> str | None:
        if recalculate:
            self._fields["flddvzeqt7FJpQ9NX"] = F.CONCATENATE("LEN: ", len(F.S(self.first_text)), "; ", "MID: ", F.MID(self.first_text, 2, 3), "; ", "LEFT: ", F.LEFT(self.second_text, 2), "; ", "RIGHT: ", F.RIGHT(self.second_text, 2), "; ", "FIND: ", F.FIND("e", self.first_text), "; ", "SEARCH: ", F.SEARCH("e", self.first_text), "; ", "REPLACE: ", F.REPLACE(self.first_text, 2, 2, "XX"), "; ", "REPT: ", (F.S(self.first_text) * 2), "; ", "LOWER: ", F.S(self.second_text).lower(), "; ", "UPPER: ", F.S(self.second_text).upper(), "; ", "TRIM: ", (("   " + F.S(self.first_text)) + "   ").strip(), "; ", "SUBSTITUTE: ", F.SUBSTITUTE(self.first_text, "e", "@"), "; ", "CONCATENATE: ", F.CONCATENATE(self.first_text, "-", self.second_text, "-", self.third_text), "; ", "T: ", F.T(self.first_number), "; ", "REGEX_EXTRACT: ", (m.group(0) if (m := re.search("[aeiou]", F.S(self.first_text))) else None), "; ", "REGEX_MATCH: ", ("1" if (bool(re.search("^.e", F.S(self.first_text)))) else "0"), "; ", "REGEX_REPLACE: ", re.sub("[aeiou]", "*", F.S(self.first_text)), "; ", "ENCODE_URL_COMPONENT: ", urllib.parse.quote(F.S(self.first_text), safe=""))
