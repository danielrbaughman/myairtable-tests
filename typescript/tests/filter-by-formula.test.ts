...
import { Airtable, FormulasModel, PrimaryModel, SecondaryModel, AND, OR, XOR, NOT } from "../output";
 ...
 * Records are tagged with a recognizable primary key so assertions can scope to this test set.
 */
describe("Filter by DateField vs DateField Formula", async () => {
	const newRecords: FormulasModel[] = [];
	...
	const scoped = (inner: string) => AND(FormulasModel.f.primaryKey.startsWith("VsField "), inner);
