/**
 * Transient 5xx retry (TypeScript) — the airtable.js gap.
 *
...
function retryConfigClass(table: object): any {
	let cls: any = table.constructor;
...
describe("create is non-idempotent: 5xx does not retry", () => {
	it("does not retry a 503 on create (single attempt)", async () => {
		...
		await expect(table.create({ fields: {} } as any)).rejects.toThrow();
