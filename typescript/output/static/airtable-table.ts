...
export interface DuplicateOptions {
	...
	typecast?: boolean;
	/**
	 * Shape of the returned copies. Defaults to the shape of the input, or `"model"` when
	 * duplicating by record id (an id carries no shape of its own).
	 */
...
> {
	...
	public async duplicate(source: Mdl, options?: DuplicateOptions): Promise<Mdl>;
	/** Duplicate multiple records */
	public async duplicate(sources: Mdl[], options?: DuplicateOptions): Promise<Mdl[]>;
	/** Duplicate a single record given an Airtable.js Record */
	public async duplicate(source: ATRecord<FldSt>, options?: DuplicateOptions): Promise<ATRecord<FldSt>>;
	/** Duplicate multiple records given Airtable.js Records */
	public async duplicate(sources: ATRecord<FldSt>[], options?: DuplicateOptions): Promise<ATRecord<FldSt>[]>;
	/** Duplicate a single record given a simple interface */
	public async duplicate(source: IRecord<FldSt>, options?: DuplicateOptions): Promise<IRecord<FldSt>>;
	/** Duplicate multiple records given simple interfaces */
	public async duplicate(sources: IRecord<FldSt>[], options?: DuplicateOptions): Promise<IRecord<FldSt>[]>;
	/** Duplicate the record with this id */
	public async duplicate(recordId: string, options?: DuplicateOptions): Promise<Mdl>;
	/** Duplicate the records with these ids */
	public async duplicate(recordIds: string[], options?: DuplicateOptions): Promise<Mdl[]>;
	/**
	...
	public async duplicate(
		source: Mdl | Mdl[] | ATRecord<FldSt> | ATRecord<FldSt>[] | IRecord<FldSt> | IRecord<FldSt>[] | string | string[],
		...
		if (isArray && (source as any[]).length === 0) return [] as any;
		const firstItem = isArray ? (source as any[])[0] : source;
		const items = (isArray ? source : [source]) as any[];
		...
		const sourceIds: string[] = typeof firstItem === "string" ? (items as string[]) : items.map((r) => r?.id);
		...
		const fetched = (await this.get(sourceIds)) as Mdl[];
		const byId = new Map<string, Mdl>(fetched.map((model) => [model.id as string, model]));
		...
		const copies =
			returnAs === "model"
				...
				: returnAs === "interface"
					? createdRecords.map((r) => this.toInterface(r))
					: createdRecords;
		return (isArray ? copies : copies[0]) as any;
	}
