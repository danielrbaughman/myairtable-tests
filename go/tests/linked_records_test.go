...
func TestLinkedRecords(t *testing.T) {
	...
	t.Run("LinkSingleAndResolve", func(t *testing.T) {
		...
		sec, err := at.Secondary.Create(ctx, &airtable.SecondaryModel{Name: airtable.String(primaryKey("Linked", "Sec"))})
		if err != nil {
			t.Fatalf("create secondary: %v", err)
		}
		defer at.Secondary.Delete(ctx, sec.ID()) //nolint:errcheck
		...
		resolved, err := at.Secondary.Get(ctx, prim.LinkSingle[0])
		if err != nil {
	...
	t.Run("LinkMultiple", func(t *testing.T) {
		s1, err := at.Secondary.Create(ctx, &airtable.SecondaryModel{Name: airtable.String(primaryKey("Linked", "M1"))})
		if err != nil {
			t.Fatalf("create s1: %v", err)
		}
		defer at.Secondary.Delete(ctx, s1.ID()) //nolint:errcheck
		s2, err := at.Secondary.Create(ctx, &airtable.SecondaryModel{Name: airtable.String(primaryKey("Linked", "M2"))})
		if err != nil {
			t.Fatalf("create s2: %v", err)
		}
		defer at.Secondary.Delete(ctx, s2.ID()) //nolint:errcheck
	...
	t.Run("EmptyLinkIsNil", func(t *testing.T) {
		prim, err := at.Primary.Create(ctx, &airtable.PrimaryModel{PrimaryKey: airtable.String(primaryKey("Linked", "Empty"))})
		if err != nil {
