...
function newPrimaryIRecord(fields = {}) {
	return { fields };
}

function newSecondaryIRecord(fields = {}) {
	return { fields };
}
...
describe("Batch Operations", async () => {
	...
	const newRecords = Array.from({ length: count }, (_, i) =>
		newPrimaryIRecord({ "Primary Key": `Batch Record ${i + 1}` }),
	);
	let ids;
	...
	describe("Read", async () => {
		const readRecords = await airtable.primary.get(ids, { returnAs: "interface" });
	...
	describe("Update", async () => {
		const fetched = await airtable.primary.get(ids, { returnAs: "interface" });
		for (let i = 0; i < fetched.length; i++) {
			fetched[i].fields["Primary Key"] = `Updated Batch Record ${i + 1}`;
		}
	...
	describe("Delete", () => {
		it("should be deleted", async () => {
			...
			const remaining = await airtable.primary.get(ids, { returnAs: "interface" });
...
describe("Invalid Record ID", async () => {
	describe("Empty String ID", async () => {
		let threw = false;
		try {
			await airtable.primary.get("", { returnAs: "interface" });
		} catch {
			threw = true;
		}
	...
	describe("Invalid ID", async () => {
		let threw = false;
		try {
			await airtable.primary.get("rec_INVALID_ID", { returnAs: "interface" });
		} catch {
			threw = true;
		}
...
describe("Upsert", async () => {
	const newRecord = newPrimaryIRecord({ "Primary Key": "Upsert Create" });
	let id;
	...
	describe("Upsert as Update", async () => {
		const r = await airtable.primary.get(id, { returnAs: "interface" });
		r.fields["Primary Key"] = "Upsert Update";
...
describe("Field Selection", async () => {
	const newRecord = newPrimaryIRecord({ "Primary Key": "Field Select", "Single Line Text": "Hello" });
	let id;
	...
	describe("Read with fields option", async () => {
		const readRecord = await airtable.primary.get(id, { fields: ["Primary Key"], returnAs: "interface" });

		it("should return the requested field", async () => {
			expect(readRecord.fields["Primary Key"]).toBe("Field Select");
		});

		it("should not return unrequested fields", async () => {
			expect(readRecord.fields["Single Line Text"]).toBeUndefined();
		});
	});
...
describe("Max Records", async () => {
	const newRecords = Array.from({ length: 5 }, (_, i) => newPrimaryIRecord({ "Primary Key": `Max Records ${i + 1}` }));
	let ids;
	...
	describe("Read with maxRecords option", async () => {
		const readRecords = await airtable.primary.get(ids, { maxRecords: 3, returnAs: "interface" });
