...
const { PrimaryModel, SecondaryModel } = require("../output");
...
describe("Simple Fields — toJson, toRecord, toIRecord", async () => {
	...
	describe("toRecord()", () => {
		...
		it("should use field IDs as keys when useFieldIds=true", () => {
			...
			expect(r.fields["fldol5Q4wmQJQvPRy"]).toBe("Serialize Simple Test");
			// "Single Line Text" field ID is "fld0BL2lFo9fqcKv3"
			expect(r.fields["fld0BL2lFo9fqcKv3"]).toBe("Hello World");
			// "Checkbox" field ID is "fldjQIaAZVegb1FUa"
			expect(r.fields["fldjQIaAZVegb1FUa"]).toBe(true);
		});
	});
	...
	describe("toIRecord()", () => {
		...
		it("should use field IDs as keys when useFieldIds=true", () => {
			...
			expect(ir.fields["fldol5Q4wmQJQvPRy"]).toBe("Serialize Simple Test");
			expect(ir.fields["fld0BL2lFo9fqcKv3"]).toBe("Hello World");
		});
	});
