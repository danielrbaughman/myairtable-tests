/**
 * TC10 - Upsert depth (JavaScript).
 *
 * Mirrors the C# `TestUpsertDepth` / Python / Rust / TypeScript suites: a multi-field merge key (a
 * match must agree on ALL merge fields), the multiple-match error path (a merge key matching more
 * than one record is rejected with 422), and a batched merge-upsert. Exercises the generated
 ...
 * not a wasCreated flag). API errors surface as a plain `Error` whose `.message` carries the 422
 * status (see error-paths.test.js).
 */
...
describe("TC10 upsert depth", () => {
	...
	it("rejects a merge key that matches multiple records (422)", async () => {
		...
		expect(String(err.message)).toContain("422");
	});
