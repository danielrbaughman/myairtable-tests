...
function newSecondaryRecord(airtable) {
...
describe("Cache Disabled By Default", async () => {
	...
	const cacheSize = airtable.primary._cache.size;
...
describe("Main Client Cascade Invalidation", async () => {
	...
	it("invalidateCache cascades to all tables", async () => {
		...
		expect(airtable.primary._cache.size).toBeGreaterThan(0);
		expect(airtable.secondary._cache.size).toBeGreaterThan(0);

		airtable.invalidateCache();
		expect(airtable.primary._cache.size).toBe(0);
		expect(airtable.secondary._cache.size).toBe(0);
	});
...
describe("Different Parameters Different Cache Keys", async () => {
	...
	it("different IDs are cached independently", async () => {
		...
		expect(airtable.primary._cache.size).toBe(2);
...
describe("Batch Get Caching", async () => {
	...
	it("batch get is cached separately from single get", async () => {
		...
		expect(airtable.primary._cache.size).toBe(1);
