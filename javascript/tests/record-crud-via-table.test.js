...
function newSecondaryRecord() {
	return new ATRecord(airtable.secondary._table, "", {});
}
...
describe("Field Selection", async () => {
	const newRecord = newPrimaryRecord();
	newRecord.set("Primary Key", "Field Select");
	...
	describe("Read with fields option", async () => {
		const readRecord = await airtable.primary.get(id, { fields: ["Primary Key"], returnAs: "record" });
...
describe("Max Records", async () => {
	const newRecords = Array.from({ length: 5 }, (_, i) => {
		const r = newPrimaryRecord();
		r.set("Primary Key", `Max Records ${i + 1}`);
		return r;
	});
	let ids;
	...
	describe("Read with maxRecords option", async () => {
		const readRecords = await airtable.primary.get(ids, { maxRecords: 3, returnAs: "record" });
