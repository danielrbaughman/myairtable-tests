...
class SecondaryTable extends AirtableTable {
	...
	/** Table name (Secondary) */
	get tableName() {
		return SecondaryTable.tableName;
	}
	...
	constructor(baseId, options) {
		super(
			...
			SecondaryWritableFieldIds,
			(record) =>
				require("../models/secondary").SecondaryModel.fromRecord(record, { baseId: this.baseId, ...this._options }),
			options,
		);
	}
}
