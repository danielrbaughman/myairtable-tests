...
class SecondaryModel extends AirtableModel {
	...
	/** Table name (Secondary) */
	get tableName() {
		return SecondaryModel.tableName;
	}
	...
	/** Table ID (tblPPScS3XMuFkDYN) */
	get tableId() {
		return SecondaryModel.tableId;
	}

	static fieldDescriptors = [
		{
			propertyName: "linkToTertiary",
			fieldId: "fldKR6tdbnOBRCtdQ",
			fieldName: "Link to Tertiary",
			isComputed: false,
			fieldType: "linkedRecords",
			linkedModelFromId: (id, config) => require("./tertiary").TertiaryModel.fromId(id, config),
		},
		{ propertyName: "name", fieldId: "fld1RagdJ09mpWhzM", fieldName: "Name", isComputed: false, fieldType: "generic" },
		{
			propertyName: "primary",
			fieldId: "fldl0nB9WRFSdqlii",
			fieldName: "Primary",
			isComputed: false,
			fieldType: "linkedRecords",
			linkedModelFromId: (id, config) => require("./primary").PrimaryModel.fromId(id, config),
		},
		{
			propertyName: "primary2",
			fieldId: "fldgoE2oZmXmKkQca",
			fieldName: "Primary 2",
			isComputed: false,
			fieldType: "linkedRecords",
			linkedModelFromId: (id, config) => require("./primary").PrimaryModel.fromId(id, config),
	...
	get linkToTertiary() {
		return this._fields["linkToTertiary"];
	}
	set linkToTertiary(value) {
		this._fields["linkToTertiary"] = value;
		this.markDirty("linkToTertiary");
	}
	...
	get primary() {
		return this._fields["primary"];
	}
	set primary(value) {
		this._fields["primary"] = value;
		this.markDirty("primary");
	}
	/** `Primary 2` (fldgoE2oZmXmKkQca) */
	get primary2() {
		return this._fields["primary2"];
	}
	set primary2(value) {
		this._fields["primary2"] = value;
		this.markDirty("primary2");
	}
	...
	constructor(data = {}) {
		...
		this.record = new (require("airtable").Record)(
			new SecondaryTable(this.getInstanceBaseId(), this.getInstanceOptions())._table,
			this.id,
			{},
		);
		this.updateRecord();
